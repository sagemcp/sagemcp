...
export default function ContainerConsole() {
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      unknown: 'bg-zinc-700 text-theme-secondary'
    }
    return colors[status?.toLowerCase()] || 'bg-zinc-700 text-theme-secondary'
  }
        ...
        <div className="p-4 border-b border-theme-default flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              onClick={loadLogs}
              disabled={loading}
              className="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 disabled:bg-zinc-700 disabled:text-theme-muted"
            >
