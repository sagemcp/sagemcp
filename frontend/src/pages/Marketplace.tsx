...
export default function Marketplace() {
  ...
  const getRuntimeColor = (runtime: string) => {
    const colors: Record<string, string> = {
      ...
      binary: 'bg-zinc-700 text-theme-secondary'
    }
    return colors[runtime.toLowerCase()] || 'bg-zinc-700 text-theme-secondary'
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-theme-elevated border border-theme-default rounded-md text-theme-primary placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-accent"
            />
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {servers.map((server) => (
                <div
                  key={server.id}
                  className="border border-theme-default rounded-lg p-4 hover:border-zinc-600 transition-colors cursor-pointer bg-theme-surface"
